...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_PAIS'))
DROP PROCEDURE BOBBY_TABLES.GET_PAIS
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.BAJA_USUARIO'))
DROP PROCEDURE BOBBY_TABLES.BAJA_USUARIO

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.HABILITAR_USUARIO'))
DROP PROCEDURE BOBBY_TABLES.HABILITAR_USUARIO

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_ESTADO_USUARIO'))
DROP PROCEDURE BOBBY_TABLES.GET_ESTADO_USUARIO

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_CLIENTE'))
DROP PROCEDURE BOBBY_TABLES.GET_CLIENTE

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_CLIENTE_ID'))
DROP PROCEDURE BOBBY_TABLES.GET_CLIENTE_ID
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.COINCIDE_DOCUMENTO'))
DROP PROCEDURE BOBBY_TABLES.COINCIDE_DOCUMENTO
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.MODIFICAR_ESTADO_CUENTA'))
DROP PROCEDURE BOBBY_TABLES.MODIFICAR_ESTADO_CUENTA
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.CREAR_FACTURA'))
DROP PROCEDURE BOBBY_TABLES.CREAR_FACTURA
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_COMISIONES_ID'))
DROP PROCEDURE BOBBY_TABLES.GET_COMISIONES_ID

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_COMISIONES_CUENTA'))
DROP PROCEDURE BOBBY_TABLES.GET_COMISIONES_CUENTA
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.COINCIDE_TARJETA'))
DROP PROCEDURE BOBBY_TABLES.COINCIDE_TARJETA

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_EMISORES'))
DROP PROCEDURE BOBBY_TABLES.GET_EMISORES
...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.CONTIENE'))
DROP FUNCTION BOBBY_TABLES.CONTIENE

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.MISMO_CLIENTE'))
DROP FUNCTION BOBBY_TABLES.MISMO_CLIENTE

IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.GET_EXPIRACION'))
DROP FUNCTION BOBBY_TABLES.GET_EXPIRACION
...
--CONTIENE LISTA_INT
GO
CREATE FUNCTION BOBBY_TABLES.CONTIENE(@ESTADOS AS BOBBY_TABLES.LISTA_INT READONLY, @ESTADO INT) RETURNS INT AS
BEGIN
	IF EXISTS (SELECT * FROM @ESTADOS WHERE ITEM = @ESTADO) RETURN 1 --SIRVE PARA EL FIND CUENTAS
	RETURN 0
END
GO
...
--MISMO CLIENTE
GO
CREATE FUNCTION BOBBY_TABLES.MISMO_CLIENTE(@CUENTA1 NUMERIC(18,0), @CUENTA2 NUMERIC(18,0)) RETURNS INT AS
BEGIN
	DECLARE @COUNT AS INT
	
	SELECT @COUNT = COUNT(CLIENTE) FROM
	(SELECT CLIENTE FROM BOBBY_TABLES.CUENTAS
	WHERE (@CUENTA1 = CUE_ID OR @CUENTA2 = CUE_ID)
	GROUP BY CLIENTE) S
...
-- GET_EXPIRACION
GO
CREATE FUNCTION BOBBY_TABLES.GET_EXPIRACION(@EXPIRACION DATETIME, @FECHA_APP DATETIME, @VIGENCIA INT) RETURNS DATETIME
BEGIN
	IF @EXPIRACION > @FECHA_APP
		RETURN @EXPIRACION + @VIGENCIA
	RETURN @FECHA_APP + @VIGENCIA
END
GO
...
INSERT INTO BOBBY_TABLES.FUNCIONALIDADES
VALUES(22, 'Consultar Saldo Administrador')
INSERT INTO BOBBY_TABLES.FUNCIONALIDADES
VALUES(23, 'ABM Usuario')
...
VALUES('Gratuita', 0, 0.5, 30)
INSERT INTO BOBBY_TABLES.TIPOS_CUENTA
VALUES('Bronce', 200, 0.2, 60)
INSERT INTO BOBBY_TABLES.TIPOS_CUENTA
VALUES('Plata', 350, 0.05, 90)
INSERT INTO BOBBY_TABLES.TIPOS_CUENTA
VALUES('Oro', 400, 0.01, 120)
...
INSERT INTO BOBBY_TABLES.USUARIOS(USERNAME, PASSWORD, FECHA_ALTA, FECHA_MODIFICACION)
SELECT DISTINCT Cli_Mail, CONVERT(VARBINARY(32),'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855',2), GETDATE(), GETDATE() FROM gd_esquema.Maestra --PASSWORD VACÍA
...
VALUES('martin', CONVERT(VARBINARY(32),'e6b87050bfcb8143fcb8db0170a4dc9ed00d904ddd3e2a4ad1b1e8dc0fdc9be7',2),GETDATE(), '¿Cuál es tu color preferido?', CONVERT(VARBINARY(32),'e6b87050bfcb8143fcb8db0170a4dc9ed00d904ddd3e2a4ad1b1e8dc0fdc9be7', 2))
INSERT INTO BOBBY_TABLES.ROLES_POR_USUARIO
VALUES(1,@@IDENTITY)
...
SELECT DISTINCT M.Cuenta_Numero, C.CLI_ID, M.Cuenta_Pais_Codigo, 1, 1, M.Cuenta_Fecha_Creacion, ISNULL(M.Cuenta_Fecha_Cierre, M.Cuenta_Fecha_Creacion + T.VIGENCIA), ISNULL(M.Cuenta_Estado, 1) FROM gd_esquema.Maestra M, BOBBY_TABLES.USUARIOS U, BOBBY_TABLES.TIPOS_CUENTA T, BOBBY_TABLES.CLIENTES C
...
INSERT INTO BOBBY_TABLES.TARJETAS(CLIENTE,TITULAR,ENCRIPTADO,LIMPIO,COD_SEGURIDAD,EMISOR,FECHA_EMISION,FECHA_VENCIMIENTO)
SELECT DISTINCT C.CLI_ID, C.NOMBRE + ' ' + C.APELLIDO, BOBBY_TABLES.ENCRIPTAR_TARJETA(M.Tarjeta_Numero), BOBBY_TABLES.ULTIMOS_4_DIGITOS(M.Tarjeta_Numero), HASHBYTES('SHA1',M.Tarjeta_Codigo_Seg), E.EMI_ID, Tarjeta_Fecha_Emision, Tarjeta_Fecha_Vencimiento FROM gd_esquema.Maestra M, BOBBY_TABLES.USUARIOS U, BOBBY_TABLES.EMISORES E, BOBBY_TABLES.CLIENTES C
WHERE C.USUARIO = U.USU_ID AND
	  U.USERNAME = M.Cli_Mail AND
...
INSERT INTO BOBBY_TABLES.CHEQUES(CHE_ID, BANCO, ACREEDOR)
SELECT DISTINCT Cheque_Numero, Banco_Cogido, 'Al portador' FROM gd_esquema.Maestra
...
SET IDENTITY_INSERT BOBBY_TABLES.DEPOSITOS ON

INSERT INTO BOBBY_TABLES.DEPOSITOS(DEP_ID, NRO_DEP, TARJETA)
SELECT O.OPE_ID, O.OPE_ID, T.TAR_ID FROM gd_esquema.Maestra M, BOBBY_TABLES.OPERACIONES O, BOBBY_TABLES.TARJETAS T
...
SET IDENTITY_INSERT BOBBY_TABLES.DEPOSITOS OFF
...
SELECT DISTINCT Cuenta_Numero, 1, Transf_Fecha, Trans_Importe, Cuenta_Dest_Numero, Item_Factura_Importe, Factura_Numero, Item_Factura_Descr, Factura_Fecha FROM gd_esquema.Maestra
...
SET IDENTITY_INSERT BOBBY_TABLES.TRANSFERENCIAS ON

INSERT INTO BOBBY_TABLES.TRANSFERENCIAS(TRA_ID, NRO_TRA, CUENTA_DESTINO, COSTO)
SELECT OPERACION, OPERACION, CUENTA_DESTINO, IMPORTE FROM BOBBY_TABLES.#AUXILIAR

SET IDENTITY_INSERT BOBBY_TABLES.TRANSFERENCIAS OFF
...
SELECT DESC_FAC, OPERACION, COSTO, FECHA_FAC, NRO_FAC FROM BOBBY_TABLES.#AUXILIAR
...
SET IDENTITY_INSERT BOBBY_TABLES.RETIROS_EFECTIVO ON

INSERT INTO BOBBY_TABLES.RETIROS_EFECTIVO(RET_ID, NRO_RET, CHEQUE)
SELECT O.OPE_ID, O.OPE_ID, C.CHE_ID FROM gd_esquema.Maestra M, BOBBY_TABLES.OPERACIONES O, BOBBY_TABLES.CHEQUES C
WHERE M.Retiro_Codigo = O.OPE_ID AND
	  C.CHE_ID = M.Cheque_Numero

SET IDENTITY_INSERT BOBBY_TABLES.RETIROS_EFECTIVO OFF
-------------------------------------------
...
GO
CREATE PROCEDURE BOBBY_TABLES.INTENTO_LOGIN(@USUARIO VARCHAR(255),@ACCESO INT,@FECHA DATETIME) AS
BEGIN --TRANSACTION
--SE DEJA ABIERTA LA POSIBILIDAD DE USAR LA FECHA DEL SISTEMA
	
	INSERT INTO BOBBY_TABLES.INTENTOS_LOGIN
	SELECT USU_ID, GETDATE(), @ACCESO FROM BOBBY_TABLES.USUARIOS
	WHERE USERNAME = @USUARIO
END --COMMIT
...
GO
CREATE PROCEDURE BOBBY_TABLES.GET_CUENTAS_USUARIO(@USUARIO INT, @ESTADO INT) AS
BEGIN
	SELECT CUE_ID, FECHA_EXPIRACION FROM BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.USUARIOS, BOBBY_TABLES.CLIENTES
	WHERE USU_ID = @USUARIO AND
		  USUARIO = USU_ID AND
		  CLIENTE = CLI_ID AND
		  (@ESTADO = 0 OR C.ESTADO = @ESTADO)
END
...
--GET PAIS
GO
CREATE PROCEDURE BOBBY_TABLES.GET_PAIS(@ID INT) AS
BEGIN
	SELECT NOMBRE FROM BOBBY_TABLES.PAISES
	WHERE PAI_ID = @ID
END
------------------------------------------------------------------------------------
----------------------------------GETS_BY_STRING------------------------------------
...
--GET PAIS
GO
CREATE PROCEDURE BOBBY_TABLES.GET_ID_PAIS(@PAIS_NOM VARCHAR(250)) AS
BEGIN --TRANSACTION
	DECLARE @PAIS_ID INT
	
	SELECT @PAIS_ID = PAI_ID FROM BOBBY_TABLES.PAISES
	WHERE @PAIS_NOM = NOMBRE
...
--BAJA USUARIO
GO
CREATE PROCEDURE BOBBY_TABLES.BAJA_USUARIO(@ID INT) AS
BEGIN
	UPDATE BOBBY_TABLES.USUARIOS SET
		ESTADO = 0
	WHERE USU_ID = @ID
...
--HABILITAR USUARIO
GO
CREATE PROCEDURE BOBBY_TABLES.HABILITAR_USUARIO(@USERNAME VARCHAR(255)) AS
BEGIN
	UPDATE BOBBY_TABLES.USUARIOS SET
		ESTADO = 1
...
--GET ESTADO USUARIO
GO
CREATE PROCEDURE BOBBY_TABLES.GET_ESTADO_USUARIO(@USERNAME VARCHAR(255)) AS
BEGIN
	DECLARE @ESTADO INT
	
	SELECT @ESTADO = ESTADO FROM BOBBY_TABLES.USUARIOS
	WHERE USERNAME = @USERNAME
...
CREATE PROCEDURE BOBBY_TABLES.AGREGAR_CLIENTE(@NOMBRE VARCHAR(255),@APELLIDO VARCHAR(255),@DOC VARCHAR(10),
	...
	@PAIS VARCHAR(250),@FECHA_NAC DATETIME,@NACIONALIDAD VARCHAR(250),@MAIL VARCHAR(255),@USERNAME VARCHAR(40),@PASSWORD_APP VARCHAR(64),
	@FECHA_ALTA DATETIME,@PREGUNTA VARCHAR(100),@RESPUESTA_APP VARCHAR(64)) AS
BEGIN --TRANSACTION
	DECLARE @PASSWORD VARBINARY(32), @RESPUESTA VARBINARY(32), @PAIS_ID INT, @NAC_ID INT, @USU_ID INT
	...
	EXEC @NAC_ID = BOBBY_TABLES.GET_ID_PAIS
				@PAIS_NOM = @NACIONALIDAD
					
	INSERT INTO BOBBY_TABLES.USUARIOS(USERNAME, PASSWORD, FECHA_ALTA, FECHA_MODIFICACION, PREGUNTA_SECRETA, RESPUESTA)
	VALUES(@USERNAME, @PASSWORD, @FECHA_ALTA, @FECHA_ALTA, @PREGUNTA, @RESPUESTA)
...
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CLIENTE(@ID INT,@NOMBRE VARCHAR(255),@APELLIDO VARCHAR(255),
	...
	@LOCALIDAD VARCHAR(60),@PAIS VARCHAR(250),@FECHA_NAC DATETIME,@NACIONALIDAD VARCHAR(250),
	@MAIL VARCHAR(255),@ESTADO INT) AS
BEGIN --TRANSACTION
	DECLARE @USUARIO INT, @PAIS_ID INT, @NAC_ID INT
	...
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN T1	--SI HAY ERROR QUIERE DECIR QUE YA EXISTÍA ESA RELACIÓN (NO HAY CAMBIO DE ESTADO)
	END	CATCH
END --COMMIT

--BAJA CLIENTES
GO
CREATE PROCEDURE BOBBY_TABLES.BAJA_CLIENTE(@ID INT) AS 
BEGIN --TRANSAC
	DECLARE @ROLES INT, @USUARIO INT
	
	UPDATE BOBBY_TABLES.CLIENTES SET
	ESTADO = 0
	WHERE CLI_ID = @ID
	
	SELECT @ROLES = COUNT(ROL), @USUARIO = R.USUARIO FROM BOBBY_TABLES.ROLES_POR_USUARIO R, BOBBY_TABLES.CLIENTES C
	WHERE R.USUARIO = C.USUARIO AND
		  CLI_ID = @ID
	GROUP BY R.USUARIO
	
	DELETE BOBBY_TABLES.ROLES_POR_USUARIO --SE BORRA EL ROL DE CLIENTE PARA EL USUARIO
	WHERE ROL = 2 AND
		  USUARIO = @USUARIO
	
	IF @ROLES = 1
		EXEC BOBBY_TABLES.BAJA_USUARIO
			@ID = @USUARIO
...
GO
CREATE PROCEDURE BOBBY_TABLES.GET_CLIENTE_ID(@USUARIO INT) AS
BEGIN
	DECLARE @CLI_ID INT
	
	SELECT @CLI_ID = CLI_ID FROM BOBBY_TABLES.CLIENTES
...
GO
CREATE PROCEDURE BOBBY_TABLES.GET_CLIENTE_DNI(@DOC VARCHAR(10),@TIPO INT) AS
BEGIN
	SELECT CLI_ID
...
BEGIN
	SELECT TAR_ID, CLIENTE, NOMBRE, TITULAR, '************' + LIMPIO AS NUMERO_TARJETA, FECHA_EMISION, FECHA_VENCIMIENTO FROM BOBBY_TABLES.TARJETAS, BOBBY_TABLES.EMISORES
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CONTRASENIA(@USUARIO INT, @PASSWORD_APP VARCHAR(64), @FECHA VARCHAR(30)) AS
BEGIN --TRANSACTION
...
GO
CREATE PROCEDURE BOBBY_TABLES.COINCIDE_DOCUMENTO(@USER INT,@TIPO INT,@NUMERO VARCHAR(10)) AS
BEGIN
	SELECT CLI_ID FROM BOBBY_TABLES.CLIENTES
...
CREATE PROCEDURE BOBBY_TABLES.AGREGAR_CUENTA(@CLIENTE INT,@MONEDA INT,@PAIS VARCHAR(250),@TIPO INT,
	@FECHA_ALTA DATETIME) AS
BEGIN --TRANSACTION
	DECLARE @PAIS_ID INT, @CUENTA NUMERIC(18,0), @VIGENCIA INT, @IMPORTE FLOAT
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_TIPO_CUENTA(@CUENTA NUMERIC(18,0),@TIPO INT,@PERIODOS INT,
	@FECHA DATETIME) AS
BEGIN --TRANSACTION
	DECLARE @PER_STR CHAR(3)
	SET @PER_STR = CONVERT(CHAR(3), @PERIODOS)
	
	DECLARE @VIGENCIA INT, @IMPORTE FLOAT
	SELECT @VIGENCIA = VIGENCIA * @PERIODOS, @IMPORTE = COSTO_MANTENIMIENTO * @PERIODOS FROM BOBBY_TABLES.TIPOS_CUENTA
	WHERE TIP_ID = @TIPO
		
	UPDATE BOBBY_TABLES.CUENTAS SET
		FECHA_EXPIRACION = BOBBY_TABLES.GET_EXPIRACION(FECHA_EXPIRACION, @FECHA, @VIGENCIA),
		TIPO = @TIPO
	WHERE CUE_ID = @CUENTA
		
	INSERT INTO BOBBY_TABLES.COMISIONES(DESCRIPCION, CUENTA, TIPO_CUENTA, IMPORTE, FECHA)
	VALUES('Cambio de tipo de cuenta por ' + @PER_STR + ' periodo/s', @CUENTA, @TIPO, @IMPORTE, @FECHA)
END --COMMIT
...
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CUENTA(@ID NUMERIC(18,0),@CLIENTE INT,@MONEDA INT,@PAIS VARCHAR(250),
	@TIPO INT,@ESTADO_APP INT,@FECHA DATETIME) AS
BEGIN --TRANSACTION
	DECLARE @TIPO_ANTERIOR INT, @ESTADO INT
	...
	IF @TIPO_ANTERIOR != @TIPO --SE CREA LA COMISION Y SE ACTUALIZA LA FECHA DE VIGENCIA SI EL TIPO DE CUENTA ESTÁ SIENDO CAMBIADO
		EXEC BOBBY_TABLES.MODIFICAR_TIPO_CUENTA
			@CUENTA = @ID,
			@TIPO = @TIPO,
			@PERIODOS = 1,
			@FECHA = @FECHA
...
--BAJA CUENTAS
GO
CREATE PROCEDURE BOBBY_TABLES.BAJA_CUENTA(@ID NUMERIC(18,0), @FECHA_CIERRE DATETIME) AS
BEGIN --TRANSACTION
	UPDATE BOBBY_TABLES.CUENTAS SET
		ESTADO = 4,
...
--MODIFICAR ESTADO CUENTAS
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_ESTADO_CUENTA(@ID NUMERIC(18,0),@ESTADO INT) AS
BEGIN
	DECLARE @ESTADO_ANT INT
	
	SELECT @ESTADO_ANT = ESTADO FROM BOBBY_TABLES.CUENTAS
	WHERE CUE_ID = @ID
...
CREATE PROCEDURE BOBBY_TABLES.FIND_CLIENTES(@NOMBRE VARCHAR(255) = NULL,@APELLIDO VARCHAR(255) = NULL, 
	@MAIL VARCHAR(255) = NULL,@TIPODOC INT,@DOC VARCHAR(10) = NULL,@ESTADO INT) AS
BEGIN
	SELECT CLI_ID AS ID, T.NOMBRE AS Tipo, NUMERO_DOC AS Numero, C.NOMBRE AS Nombre, APELLIDO AS Apellido, P.NOMBRE AS Residencia, N.NOMBRE AS Nacionalidad, FECHA_NACIMIENTO AS 'Fecha Nacimiento', U.USERNAME AS Usuario, MAIL AS Mail  FROM BOBBY_TABLES.CLIENTES C, BOBBY_TABLES.TIPOS_DOCUMENTO T, BOBBY_TABLES.PAISES P, BOBBY_TABLES.USUARIOS U, BOBBY_TABLES.PAISES N
	WHERE (@NOMBRE IS NULL OR C.NOMBRE LIKE '%' + @NOMBRE + '%') AND
		  (@APELLIDO IS NULL OR APELLIDO LIKE '%' + @APELLIDO + '%') AND
		  (@MAIL IS NULL OR MAIL LIKE '%' + @MAIL + '%') AND
		  (@TIPODOC = 0 OR TIPO_DOC = @TIPODOC) AND
		  (@DOC IS NULL OR NUMERO_DOC LIKE '%' + @DOC + '%') AND
		  (@ESTADO = 0 OR C.ESTADO = @ESTADO) AND
		  C.USUARIO = U.USU_ID AND
		  C.PAIS = P.PAI_ID AND
		  C.NACIONALIDAD = N.PAI_ID AND
		  C.TIPO_DOC = T.TIP_ID
...
CREATE PROCEDURE BOBBY_TABLES.FIND_CUENTAS(@NUMERO VARCHAR(16) = NULL,@PAIS VARCHAR(50) = NULL,
	@TIPO INT = NULL,@ESTADOS AS BOBBY_TABLES.LISTA_INT READONLY) AS
BEGIN
	SELECT CUE_ID AS Numero, CL.NOMBRE + ' ' + CL.APELLIDO AS Cliente, P.NOMBRE AS Pais, M.NOMBRE AS Moneda, T.NOMBRE AS Tipo, E.NOMBRE AS Estado, SALDO AS Saldo, FECHA_ALTA AS 'Fecha Alta', FECHA_EXPIRACION AS 'Fecha Expiración', FECHA_CIERRE AS 'Fecha Cierre' FROM BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.PAISES P, BOBBY_TABLES.TIPOS_CUENTA T, BOBBY_TABLES.CLIENTES CL, BOBBY_TABLES.MONEDAS M, BOBBY_TABLES.ESTADOS_CUENTA E
	WHERE  C.PAIS = P.PAI_ID AND
		  (@NUMERO IS NULL OR CUE_ID LIKE '%' + @NUMERO + '%') AND
		  (@PAIS IS NULL OR P.NOMBRE LIKE '%' + @PAIS + '%') AND
		  (@TIPO = 0 OR TIPO = @TIPO) AND
		  BOBBY_TABLES.CONTIENE(@ESTADOS, C.ESTADO) = 1 AND
		  C.CLIENTE = CLI_ID AND
		  C.ESTADO = EST_ID AND
		  C.MONEDA = MON_ID AND
		  C.TIPO = TIP_ID
END
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_COSTOS_TIPO(@ID INT,@COSTO_MANTENIMIENTO FLOAT,
	@COSTO_TRANSFERENCIA FLOAT,@VIGENCIA INT) AS
BEGIN --TRANSACTION
...
--ALTA ROLES
GO
CREATE PROCEDURE BOBBY_TABLES.AGREGAR_ROL(@NOMBRE VARCHAR(50), @FUNCIONALIDADES AS BOBBY_TABLES.LISTA_INT READONLY,
	@ESTADO INT) AS
BEGIN --TRANSACTION
	DECLARE @ID_ROL INT
	
	INSERT INTO BOBBY_TABLES.ROLES(NOMBRE, ESTADO)
...
--MODIFICACIÓN ROLES
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_ROL(@ID INT, @NOMBRE VARCHAR(50), @FUNCIONALIDADES AS BOBBY_TABLES.LISTA_INT READONLY,
	@ESTADO INT) AS
BEGIN --TRANSACTION
...
BEGIN
	SELECT ROL_ID, NOMBRE, FUNCIONALIDAD, ESTADO FROM BOBBY_TABLES.ROLES, BOBBY_TABLES.FUNCIONALIDADES_POR_ROL
	WHERE ROL_ID = @ID AND
		  ROL_ID = ROL
END
...
GO
CREATE PROCEDURE BOBBY_TABLES.GET_ROLES(@ESTADO INT) AS
BEGIN
...
BEGIN
	SELECT FUN_ID, NOMBRE AS Funcionalidad FROM BOBBY_TABLES.FUNCIONALIDADES_POR_ROL, BOBBY_TABLES.FUNCIONALIDADES
	WHERE ROL = @ROL_ID AND
		  FUN_ID = FUNCIONALIDAD
END
...
--ASOCIACIÓN TARJETAS
GO
CREATE PROCEDURE BOBBY_TABLES.ASOCIAR_TARJETA(@NUMERO NUMERIC(16,0),@CLIENTE INT,@TITULAR VARCHAR(50),
	@CODIGO_SEG NUMERIC(3,0),@EMISOR VARCHAR(60),@FECHA_EMISION DATETIME,@FECHA_VENCIMIENTO DATETIME) AS
BEGIN --TRANSACTION
	DECLARE @EMISOR_ID INT
	...
	INSERT INTO BOBBY_TABLES.TARJETAS(CLIENTE,TITULAR,ENCRIPTADO,LIMPIO,COD_SEGURIDAD,EMISOR,FECHA_EMISION,FECHA_VENCIMIENTO)
	VALUES(@CLIENTE, @TITULAR, BOBBY_TABLES.ENCRIPTAR_TARJETA(@NUMERO), BOBBY_TABLES.ULTIMOS_4_DIGITOS(@NUMERO), HASHBYTES('SHA1',CONVERT(VARCHAR(4),@CODIGO_SEG)), @EMISOR_ID, @FECHA_EMISION, @FECHA_VENCIMIENTO)
END --COMMIT

--MODIFICIÓN TARJETAS
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_TARJETA(@ID INT,@NUMERO NUMERIC(16,0),@TITULAR VARCHAR(50),
	@CODIGO_SEG NUMERIC(3,0),@EMISOR VARCHAR(60),@FECHA_EMISION DATETIME,
	@FECHA_VENCIMIENTO DATETIME,@ESTADO INT) AS
BEGIN --TRANSACTION
	DECLARE @EMISOR_ID INT
...
BEGIN
	SELECT T.*, E.NOMBRE AS NOMBRE FROM BOBBY_TABLES.TARJETAS T, BOBBY_TABLES.EMISORES E
	WHERE TAR_ID = @ID AND
		  EMI_ID = EMISOR
END
...
--GET ID TARJETAS
GO
CREATE PROCEDURE BOBBY_TABLES.GET_ID_TARJETA(@ENCRIPT VARCHAR(40),@ULTIMOS VARCHAR(4),@COD_SEG VARCHAR(40),@FECHA DATETIME,@USER INT) AS
BEGIN
	DECLARE @ID INT, @HASH_ENC VARBINARY(20), @HASH_COD VARBINARY(20), @LIMPIO NUMERIC(4,0)
...
GO
CREATE PROCEDURE BOBBY_TABLES.GET_TARJETAS(@USER INT, @FECHA DATETIME,@ESTADO SMALLINT) AS
BEGIN
	SELECT TAR_ID, '************' + LIMPIO AS NUMERO FROM BOBBY_TABLES.TARJETAS T, BOBBY_TABLES.CLIENTES
	WHERE USUARIO = @USER AND
		  CLIENTE = CLI_ID AND
		  FECHA_VENCIMIENTO >= @FECHA AND
		  (@ESTADO = 2 OR T.ESTADO = @ESTADO)
END

--COINCIDE TARJETA
GO
CREATE PROCEDURE BOBBY_TABLES.COINCIDE_TARJETA(@ID INT,@ENCRIPT VARCHAR(40),@ULTIMOS VARCHAR(4),
	@COD VARCHAR(40)) AS
BEGIN
	DECLARE @ENCRIPTADO VARBINARY(20), @CODSEG VARBINARY(20)
	SET @ENCRIPTADO = CONVERT(VARBINARY(20), @ENCRIPT, 2)
	SET @CODSEG = CONVERT(VARBINARY(20), @COD, 2)
	
	SELECT TAR_ID FROM BOBBY_TABLES.TARJETAS
	WHERE TAR_ID = @ID AND
		  ENCRIPTADO = @ENCRIPTADO AND
		  LIMPIO = @ULTIMOS AND
		  COD_SEGURIDAD = @CODSEG
END

--GET EMISORES
GO
CREATE PROCEDURE BOBBY_TABLES.GET_EMISORES AS
BEGIN
	SELECT * FROM BOBBY_TABLES.EMISORES
END
------------------------------------------------------------------------------------
-----------------------------------OPERACIONES--------------------------------------
------------------------------------------------------------------------------------
...
--RETIRO DE EFECTIVO
GO
CREATE PROCEDURE BOBBY_TABLES.RETIRAR_EFECTIVO(@CUENTA NUMERIC(18,0),@FECHA DATETIME,
	@IMPORTE FLOAT,@MONEDA INT,@BANCO VARCHAR(50),@ACREEDOR VARCHAR(100)) AS
BEGIN --TRANSACTION
	DECLARE @BANCO_ID INT, @CHEQUE INT
	
	EXEC @BANCO_ID = BOBBY_TABLES.GET_ID_BANCO
					@BANCO = @BANCO
	
	INSERT INTO BOBBY_TABLES.CHEQUES(BANCO, ACREEDOR)
	VALUES(@BANCO_ID, @ACREEDOR)
...
CREATE PROCEDURE BOBBY_TABLES.TRANSFERIR(@ORIGEN NUMERIC(18,0),@DESTINO NUMERIC(18,0),
	@FECHA DATETIME,@IMPORTE FLOAT,@MONEDA INT) AS
BEGIN --TRANSACTION
	DECLARE @COSTO_TRANSF FLOAT, @TIPO_CUENTA INT, @OPERACION NUMERIC(18,0)
	...
	IF BOBBY_TABLES.MISMO_CLIENTE(@ORIGEN, @DESTINO) = 1
		SET @COSTO_TRANSF = 0
	ELSE
	BEGIN
		SELECT @COSTO_TRANSF = T.COSTO_TRANSACCION * @IMPORTE, @TIPO_CUENTA = T.TIP_ID FROM BOBBY_TABLES.TIPOS_CUENTA T, BOBBY_TABLES.CUENTAS C
		WHERE C.TIPO = T.TIP_ID
		
		INSERT INTO BOBBY_TABLES.COMISIONES(DESCRIPCION, OPERACION, CUENTA, TIPO_CUENTA, IMPORTE, FECHA)
		VALUES('Costos de transferencia', @OPERACION, @ORIGEN, @TIPO_CUENTA, @COSTO_TRANSF, @FECHA)
	END
...
--FACTURAR
GO
CREATE PROCEDURE BOBBY_TABLES.CREAR_FACTURA(@COMISIONES_ID AS BOBBY_TABLES.LISTA_INT READONLY,
	@FECHA DATETIME,@TARJETA VARCHAR(40),@TOTAL FLOAT) AS
BEGIN
	DECLARE @TARJETA_HASH VARBINARY(20), @FACTURA NUMERIC(18,0)
	SET @TARJETA_HASH = CONVERT(VARBINARY(20),@TARJETA, 2)
	
	INSERT INTO BOBBY_TABLES.FACTURAS
	VALUES(@TARJETA_HASH, 1, @TOTAL)
	
	SET @FACTURA = @@IDENTITY
	
	UPDATE BOBBY_TABLES.COMISIONES SET
		FACTURA = @FACTURA
	WHERE FACTURA IS NULL AND
		  BOBBY_TABLES.CONTIENE(@COMISIONES_ID, COM_ID) = 1
...
BEGIN
	SELECT F.COM_ID AS ID, C.CUE_ID AS Cuenta, F.DESCRIPCION AS Descripción, F.FECHA AS Fecha, F.IMPORTE AS Importe, T.NOMBRE AS 'Tipo Cuenta' FROM BOBBY_TABLES.COMISIONES F, BOBBY_TABLES.CLIENTES P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.TIPOS_CUENTA T
	WHERE (@ID = 0 OR P.USUARIO = @ID) AND
		  F.CUENTA = C.CUE_ID AND
		  P.CLI_ID = C.CLIENTE AND
		  F.TIPO_CUENTA = T.TIP_ID AND
		  F.FACTURA IS NULL
END

--COMISIONES POR IDS
GO
CREATE PROCEDURE BOBBY_TABLES.GET_COMISIONES_ID(@IDS AS BOBBY_TABLES.LISTA_INT READONLY) AS
BEGIN
	SELECT F.COM_ID AS ID, C.CUE_ID AS Cuenta, F.DESCRIPCION AS Descripción, F.FECHA AS Fecha, F.IMPORTE AS Importe, T.NOMBRE AS 'Tipo Cuenta' FROM BOBBY_TABLES.COMISIONES F, BOBBY_TABLES.CLIENTES P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.TIPOS_CUENTA T
	WHERE BOBBY_TABLES.CONTIENE(@IDS,COM_ID) = 1 AND
		  F.CUENTA = C.CUE_ID AND
		  P.CLI_ID = C.CLIENTE AND
		  F.TIPO_CUENTA = T.TIP_ID AND
		  F.FACTURA IS NULL
...
--COMISIONES ADEUDADAS POR CUENTA
GO
CREATE PROCEDURE BOBBY_TABLES.GET_COMISIONES_CUENTA(@CUENTA NUMERIC(18,0)) AS
BEGIN
	SELECT COM_ID FROM BOBBY_TABLES.COMISIONES
...
BEGIN
	SELECT @CUENTA AS 'Cuenta Origen', C.CUE_ID AS 'Cuenta Destino', P.NOMBRE + ' ' + P.APELLIDO AS Cliente, O.IMPORTE AS Importe, T.COSTO AS Costo, M.NOMBRE AS Moneda, O.FECHA AS Fecha FROM BOBBY_TABLES.OPERACIONES O, BOBBY_TABLES.TRANSFERENCIAS T, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.CLIENTES P, BOBBY_TABLES.MONEDAS M
	WHERE O.CUENTA = @CUENTA AND
		  ...
		  O.MONEDA = M.MON_ID
	UNION
	SELECT C.CUE_ID AS 'Cuenta Origen', @CUENTA AS 'Cuenta Destino', P.NOMBRE + ' ' + P.APELLIDO AS Cliente, O.IMPORTE AS Importe, T.COSTO AS Costo, M.NOMBRE AS Moneda, O.FECHA AS Fecha FROM BOBBY_TABLES.OPERACIONES O, BOBBY_TABLES.TRANSFERENCIAS T, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.CLIENTES P, BOBBY_TABLES.MONEDAS M
...
GO
CREATE PROCEDURE BOBBY_TABLES.CLIENTES_CON_CUENTAS_INHABILITADAS_POR_AUSENCIA_DE_PAGO(@FECHA_INICIO DATETIME, @FECHA_FIN DATETIME) AS
BEGIN
	SELECT TOP 5 P.CLI_ID AS ID_Cliente, P.NOMBRE AS Nombre, P.APELLIDO AS Apellido, COUNT(C.CUE_ID) AS Comisiones, SUM(F.IMPORTE) AS Deuda_Total FROM BOBBY_TABLES.CLIENTES P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.COMISIONES F
	WHERE P.CLI_ID = C.CLIENTE AND
		  C.ESTADO = 2 AND --2 DESHABILITADA
		  F.CUENTA = C.CUE_ID AND
		  F.FACTURA IS NULL --AND
		  --F.FECHA BETWEEN @FECHA_INICIO AND @FECHA_FIN
	GROUP BY P.CLI_ID, P.NOMBRE, P.APELLIDO
	ORDER BY DEUDA_TOTAL DESC, COMISIONES DESC
END


--PUNTO 2 LISTADO ESTADÍSTICO (SE INTERPRETA COMO COMISIÓN A LOS COSTOS GENERADOS POR TRANSFERENCIAS)
GO
CREATE PROCEDURE BOBBY_TABLES.CLIENTES_CON_MAS_COMISIONES_FACTURADAS(@FECHA_INICIO DATETIME, @FECHA_FIN DATETIME) AS
BEGIN
...
GO
CREATE PROCEDURE BOBBY_TABLES.CLIENTES_CON_MAYOR_TRANSFERENCIA_ENTRE_CUENTAS_PROPIAS(@FECHA_INICIO DATETIME, @FECHA_FIN DATETIME) AS
BEGIN
...
GO
CREATE PROCEDURE BOBBY_TABLES.PAISES_CON_MAS_MOVIMIENTOS(@FECHA_INICIO DATETIME, @FECHA_FIN DATETIME) AS
BEGIN
...
GO
CREATE PROCEDURE BOBBY_TABLES.FACTURADO_PARA_CADA_TIPO(@FECHA_INICIO DATETIME, @FECHA_FIN DATETIME) AS
BEGIN
...
SELECT * FROM BOBBY_TABLES.COMISIONES