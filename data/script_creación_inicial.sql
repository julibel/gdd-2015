...
CREATE TABLE BOBBY_TABLES.DEPOSITOS(
	DEP_ID NUMERIC(20,0) PRIMARY KEY,
	TARJETA INT
)

CREATE TABLE BOBBY_TABLES.RETIROS_EFECTIVO(
	RET_ID NUMERIC(20,0) PRIMARY KEY,
	CHEQUE INT
)

CREATE TABLE BOBBY_TABLES.TRANSFERENCIAS(
	TRA_ID NUMERIC(20,0) PRIMARY KEY,
	CUENTA_DESTINO NUMERIC(18,0),
	COSTO FLOAT
)

CREATE TABLE BOBBY_TABLES.OPERACIONES(
	OPE_ID NUMERIC(20,0) IDENTITY(1000000000,1) PRIMARY KEY,
	CUENTA NUMERIC(18,0),
	MONEDA INT,
	FECHA DATE,
	IMPORTE FLOAT
...
SELECT DISTINCT U.USU_ID, BOBBY_TABLES.ENCRIPTAR_TARJETA(M.Tarjeta_Numero), BOBBY_TABLES.ULTIMOS_4_DIGITOS(M.Tarjeta_Numero), M.Tarjeta_Codigo_Seg, E.EMI_ID, Tarjeta_Fecha_Emision, Tarjeta_Fecha_Vencimiento FROM gd_esquema.Maestra M, BOBBY_TABLES.USUARIOS U, BOBBY_TABLES.EMISORES E
...
SET IDENTITY_INSERT BOBBY_TABLES.OPERACIONES ON --PERMITO INSERTAR VALORES EN IDENTITY
...
SET IDENTITY_INSERT BOBBY_TABLES.OPERACIONES OFF --CORTO EL PERMISO ESTABLECIDO
...
DECLARE @INICIO NUMERIC(20,0)
DECLARE @FIN NUMERIC(20,0)
SET @INICIO = 1000000000 --SIRVEN PARA MANEJAR EL MISMO ID ENTRE TRANSFERENCIAS Y OPERACIONES

INSERT INTO BOBBY_TABLES.OPERACIONES
SELECT DISTINCT Cuenta_Numero, 1, Transf_Fecha, Trans_Importe FROM gd_esquema.Maestra
...
DECLARE @INICIO NUMERIC(20,0)
DECLARE @FIN NUMERIC(20,0)
SET @INICIO = 1000000000

SET @FIN = @@IDENTITY
/*
--AGREGANDO TRANSFERENCIAS DE LA TABLA OPERACIONES EN TRASFERENCIAS
INSERT INTO BOBBY_TABLES.TRANSFERENCIAS
SELECT DISTINCT * FROM
	(SELECT O.OPE_ID, M.Cuenta_Dest_Numero, M.Trans_Costo_Trans FROM gd_esquema.Maestra M, BOBBY_TABLES.OPERACIONES O
		WHERE OPE_ID BETWEEN @INICIO AND @FIN AND
		M.Transf_Fecha = O.FECHA AND
		M.Cuenta_Numero = O.CUENTA AND
		M.Trans_Importe = O.IMPORTE) AS TRANS
*/
...
SELECT DISTINCT Factura_Numero FROM gd_esquema.Maestra
