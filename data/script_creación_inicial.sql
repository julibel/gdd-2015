...
INSERT INTO BOBBY_TABLES.CLIENTES(TIPO_DOC, NUMERO_DOC, NOMBRE, APELLIDO, PAIS, CALLE, PISO, DEPTO, FECHA_NACIMIENTO, MAIL, USUARIO)
SELECT DISTINCT 5, M.Cli_Nro_Doc, M.Cli_Nombre, M.Cli_Apellido, M.Cli_Pais_Codigo, M.Cli_Dom_Calle + ' ' + CONVERT(VARCHAR(10),M.Cli_Dom_Nro), M.Cli_Dom_Piso, M.Cli_Dom_Depto, M.Cli_Fecha_Nac, M.Cli_Mail, U.USU_ID FROM gd_esquema.Maestra M, BOBBY_TABLES.USUARIOS U --HARDCODEO EL PASAPORTE PARA SIMPLIFICAR LA APLICACIÓN
...
CREATE PROCEDURE BOBBY_TABLES.AGREGAR_CLIENTE(@NOMBRE VARCHAR(255),@APELLIDO VARCHAR(255),@DOC VARCHAR(10),
	...
	@PAIS VARCHAR(250),@FECHA_NAC_APP VARCHAR(100),@NACIONALIDAD VARCHAR(250),@MAIL VARCHAR(255),@USERNAME VARCHAR(40),@PASSWORD_APP VARCHAR(64),
...
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CLIENTE(@ID INT,@NOMBRE VARCHAR(255),@APELLIDO VARCHAR(255),
	@DOC NUMERIC(10,0),@TIPO_DOC INT,@CALLE VARCHAR(255),@PISO INT,@DEPTO VARCHAR(10),
...
END
	...
	SELECT TOP 5 1111111111111267 AS Cuenta, O.IMPORTE AS Importe, M.NOMBRE AS Moneda, O.FECHA AS Fecha, '************' + T.LIMPIO AS Tarjeta FROM BOBBY_TABLES.OPERACIONES O, BOBBY_TABLES.DEPOSITOS D, BOBBY_TABLES.TARJETAS T, BOBBY_TABLES.MONEDAS M
	WHERE O.CUENTA = 1111111111111267 AND
		  O.OPE_ID = D.DEP_ID AND
		  D.TARJETA = T.TAR_ID AND
		  O.MONEDA = M.MON_ID
	ORDER BY Fecha DESC
	
	select * from BOBBY_TABLES.CUENTAS
	where BOBBY_TABLES.ULTIMOS_4_DIGITOS(CUE_ID) = '1267'
	
	select * from BOBBY_TABLES.CLIENTES
	
	SELECT * FROM BOBBY_TABLES.USUARIOS
	
	EXEC BOBBY_TABLES.AGREGAR_CLIENTE
				@NOMBRE = 'MARTIN',
				@APELLIDO = 'GOTELLI',
				@DOC = '37952200',
				@TIPO_DOC = 1,
				...
				@LOCALIDAD = 'CABA',
				@PAIS = 'Argentina',
				@FECHA_NAC_APP = '11/11/2017',
				@NACIONALIDAD = 'Argentina',
				@MAIL = 'martinpiola@gotmail.com',
				@USERNAME = 'martinchito',
				@PASSWORD_APP = 'E6B87050BFCB8143FCB8DB0170A4DC9ED00D904DDD3E2A4AD1B1E8DC0FDC9BE7',
				@FECHA_ALTA_APP = '11/11/2017',
				@PREGUNTA = 'Quién eres tu?',
				@RESPUESTA_APP = 'E6B87050BFCB8143FCB8DB0170A4DC9ED00D904DDD3E2A4AD1B1E8DC0FDC9BE7'
				
				SELECT * FROM BOBBY_TABLES.CLIENTES
				where NOMBRE like '%lvi%'