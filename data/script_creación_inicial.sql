...
IF EXISTS(SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('BOBBY_TABLES.ENCRIPTAR_TARJETA') AND OBJECTPROPERTY(id, 'IsUserFunction') = 1)
DROP TABLE BOBBY_TABLES.ENCRIPTAR_TARJETA
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CONTRASEÑA(@USUARIO INT, @PASSWORD VARBINARY(32)) AS
BEGIN
	UPDATE BOBBY_TABLES.USUARIOS SET
		PASSWORD = @PASSWORD
...
--ALTA CUENTAS
GO
CREATE PROCEDURE BOBBY_TABLES.AGREGAR_CUENTA(@USUARIO INT,@MONEDA INT,@PAIS VARCHAR(250),@TIPO INT,
...
BEGIN
	...
	VALUES(@USUARIO, @PAIS, @MONEDA, @TIPO, @FECHA_ALTA, 3) --ESTADO PENDIENTE DE ACTIVACIÓN
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CUENTA(@ID NUMERIC(18,0),@USUARIO INT,@MONEDA INT,@PAIS VARCHAR(250),
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_TIPO(@ID INT,@COSTO_MANTENIMIENTO FLOAT,@COSTO_TRANSFERENCIA FLOAT,
	@VIGENCIA INT) AS
BEGIN
...
BEGIN--http://stackoverflow.com/questions/11102358/how-to-pass-an-array-into-a-sql-server-stored-procedure
	DECLARE @ID_ROL INT
...
BEGIN --http://stackoverflow.com/questions/11102358/how-to-pass-an-array-into-a-sql-server-stored-procedure
	...
	SELECT @ID, ITEM FROM @FUNCIONALIDADES
END
...
--ASOCIACIÓN TARJETAS
GO
CREATE PROCEDURE BOBBY_TABLES.ASOCIAR_TARJETA(@NUMERO NUMERIC(16,0),@USUARIO INT,@CODIGO_SEG NUMERIC(4,0),
...
BEGIN
	...
	INSERT INTO BOBBY_TABLES.TARJETAS
	VALUES(@USUARIO, BOBBY_TABLES.ENCRIPTAR_TARJETA(@NUMERO), BOBBY_TABLES.ULTIMOS_4_DIGITOS(@NUMERO), HASHBYTES('SHA1',@CODIGO_SEG), @EMISOR_ID, @FECHA_EMISION, @FECHA_VENCIMIENTO)
END

--MODIFICIÓN TARJETAS
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_TARJETA(@ID INT,@NUMERO NUMERIC(16,0),@USUARIO INT,@CODIGO_SEG NUMERIC(4,0),
	@EMISOR VARCHAR(60),@FECHA_EMISION_APP VARCHAR(30),@FECHA_VENCIMIENTO_APP VARCHAR(30)) AS
BEGIN
	DECLARE @EMISOR_ID INT, @FECHA_EMISION DATETIME, @FECHA_VENCIMIENTO DATETIME
	SET @FECHA_EMISION = CONVERT(DATETIME, @FECHA_EMISION_APP)
	...
	UPDATE BOBBY_TABLES.TARJETAS SET
		...
		COD_SEGURIDAD = HASHBYTES('SHA1',@CODIGO_SEG),
		EMISOR = @EMISOR_ID,
...
--GET TARJETAS
GO
CREATE FUNCTION BOBBY_TABLES.GET_TARJETA(@ID INT) RETURNS @TARJETA TABLE(
	TAR_ID INT,
	USUARIO INT,
	ENCRIPTADO VARBINARY(20),
	LIMPIO NUMERIC(4,0),
...
BEGIN
	...
	VALUES(@CUENTA, @MONEDA, @IMPORTE, @FECHA)
...
GO
CREATE FUNCTION BOBBY_TABLES.CLIENTES_CON_CUENTAS_INHABILITADAS_POR_AUSENCIA_DE_PAGO(@FECHA_INICIO_APP VARCHAR(30), @FECHA_FIN_APP VARCHAR(30)) RETURNS @CLIENTES TABLE(
	PER_ID INT,
	NOMBRE VARCHAR(255),
	APELLIDO VARCHAR(255),
	CUENTAS INT,
...
BEGIN
	...
	SELECT TOP 5 P.PER_ID, P.NOMBRE, P.APELLIDO, COUNT(C.CUE_ID) AS CUENTAS, SUM(F.IMPORTE) AS DEUDA_TOTAL FROM BOBBY_TABLES.PERSONAS P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.FACTURAS F, BOBBY_TABLES.ESTADOS_CUENTA E
	WHERE P.USUARIO = C.USUARIO AND
		  C.ESTADO = E.EST_ID AND
		  E.NOMBRE = 'Deshabilitada' AND
		  F.CUENTA = C.CUE_ID AND
		  F.PAGADO = 0 AND
		  F.FECHA BETWEEN @FECHA_INICIO AND @FECHA_FIN
	GROUP BY P.PER_ID, P.NOMBRE, P.APELLIDO
...
GO
CREATE FUNCTION BOBBY_TABLES.CLIENTES_CON_MAS_COMISIONES_FACTURADAS(@FECHA_INICIO_APP VARCHAR(30), @FECHA_FIN_APP VARCHAR(30)) RETURNS @CLIENTES TABLE(
	PER_ID INT,
	NOMBRE VARCHAR(255),
	APELLIDO VARCHAR(255),
...
BEGIN
	...
	INSERT INTO @CLIENTES
	SELECT TOP 5 P.PER_ID, P.NOMBRE, P.APELLIDO, COUNT(T.TRA_ID) AS CANTIDAD_COMISIONES, SUM(F.IMPORTE) AS TOTAL_COMISIONES FROM BOBBY_TABLES.PERSONAS P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.TRANSFERENCIAS T, BOBBY_TABLES.OPERACIONES O, BOBBY_TABLES.FACTURAS F
	WHERE P.USUARIO = C.USUARIO AND
...
GO
CREATE FUNCTION BOBBY_TABLES.CLIENTES_CON_MAYOR_TRANSFERENCIA_ENTRE_CUENTAS_PROPIAS(@FECHA_INICIO_APP VARCHAR(30), @FECHA_FIN_APP VARCHAR(30)) RETURNS @CLIENTES TABLE(
	PER_ID INT,
	NOMBRE VARCHAR(255),
	APELLIDO VARCHAR(255),
...
BEGIN
	...
	INSERT INTO @CLIENTES
	SELECT TOP 5 P.PER_ID, P.NOMBRE, P.APELLIDO, COUNT(T.TRA_ID) AS CANTIDAD_TRANSFERENCIAS FROM BOBBY_TABLES.PERSONAS P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.TRANSFERENCIAS T, BOBBY_TABLES.OPERACIONES O
	WHERE P.USUARIO = C.USUARIO AND
		  O.OPE_ID = T.TRA_ID AND
		  O.CUENTA = C.CUE_ID AND
...
GO
CREATE FUNCTION BOBBY_TABLES.PAISES_CON_MAS_MOVIMIENTOS(@FECHA_INICIO_APP VARCHAR(30), @FECHA_FIN_APP VARCHAR(30)) RETURNS @PAISES TABLE(
	PAI_ID INT,
	NOMBRE VARCHAR(250),
...
GO
CREATE FUNCTION BOBBY_TABLES.FACTURADO_PARA_CADA_TIPO(@FECHA_INICIO_APP VARCHAR(30), @FECHA_FIN_APP VARCHAR(30)) RETURNS @TIPOS_CUENTA TABLE(
	TIP_ID INT,
	NOMBRE VARCHAR(9),
	FACTURADO FLOAT
) AS
