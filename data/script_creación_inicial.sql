...
GO
CREATE PROCEDURE BOBBY_TABLES.INTENTO_LOGIN(@USUARIO VARCHAR(255),@ACCESO INT,@FECHA/*_APP VARCHAR(30)*/DATETIME) AS
BEGIN --TRANSACTION
	--DECLARE @FECHA DATETIME
	--SET @FECHA = CONVERT(DATETIME, @FECHA_APP) --SE DEJA ABIERTA LA POSIBILIDAD DE USAR LA FECHA DEL SISTEMA
...
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CLIENTE(@ID INT,@NOMBRE VARCHAR(255),@APELLIDO VARCHAR(255),
	...
	@LOCALIDAD VARCHAR(60),@PAIS VARCHAR(250),@FECHA_NAC_APP VARCHAR(30),@NACIONALIDAD VARCHAR(250),
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CONTRASENIA(@USUARIO INT, @PASSWORD_APP VARCHAR(64), @FECHA_APP VARCHAR(30)) AS
BEGIN --TRANSACTION
	DECLARE @PASSWORD VARBINARY(32), @FECHA DATETIME
	SET @PASSWORD = CONVERT(VARBINARY(32), @PASSWORD_APP, 2)
...
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_CUENTA(@ID NUMERIC(18,0),@CLIENTE INT,@MONEDA INT,@PAIS VARCHAR(250),
	@TIPO INT,@ESTADO_APP INT,@FECHA_APP VARCHAR(30)) AS
BEGIN --TRANSACTION
	DECLARE @TIPO_ANTERIOR INT, @FECHA DATETIME, @ESTADO INT
	SET @FECHA = CONVERT(DATETIME, @FECHA_APP)
	IF @ESTADO_APP = 1
		SET @ESTADO = 1
	ELSE
...
GO
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_TIPO_CUENTA(@CUENTA NUMERIC(18,0),@TIPO INT,@PERIODOS INT,@FECHA_APP VARCHAR(30)) AS
BEGIN --TRANSACTION
	DECLARE @FECHA DATETIME, @PER_STR CHAR(3)
	SET @FECHA = CONVERT(DATETIME, @FECHA_APP)
...
CREATE PROCEDURE BOBBY_TABLES.ASOCIAR_TARJETA(@NUMERO NUMERIC(16,0),@CLIENTE INT,@TITULAR VARCHAR(50),
	@CODIGO_SEG NUMERIC(3,0),@EMISOR VARCHAR(60),@FECHA_EMISION_APP VARCHAR(30),@FECHA_VENCIMIENTO_APP VARCHAR(30)) AS
BEGIN --TRANSACTION
...
CREATE PROCEDURE BOBBY_TABLES.MODIFICAR_TARJETA(@ID INT,@NUMERO NUMERIC(16,0),@TITULAR VARCHAR(50),
	@CODIGO_SEG NUMERIC(3,0),@EMISOR VARCHAR(60),@FECHA_EMISION_APP VARCHAR(30),
	@FECHA_VENCIMIENTO_APP VARCHAR(30),@ESTADO INT) AS
BEGIN --TRANSACTION
	DECLARE @EMISOR_ID INT, @FECHA_EMISION DATETIME, @FECHA_VENCIMIENTO DATETIME
	SET @FECHA_EMISION = CONVERT(DATETIME, @FECHA_EMISION_APP)
	SET @FECHA_VENCIMIENTO = CONVERT(DATETIME, @FECHA_VENCIMIENTO_APP)
...
GO
CREATE PROCEDURE BOBBY_TABLES.GET_TARJETAS(@USER INT, @FECHA_APP VARCHAR(30),@ESTADO SMALLINT) AS
BEGIN
	DECLARE @FECHA DATETIME
	SET @FECHA = CONVERT(DATETIME, @FECHA_APP)
...
--COINCIDE TARJETA
GO
CREATE PROCEDURE BOBBY_TABLES.COINCIDE_TARJETA(@ID INT,@ENCRIPT VARCHAR(40),@ULTIMOS VARCHAR(4),@COD VARCHAR(40)) AS
BEGIN
...
CREATE PROCEDURE BOBBY_TABLES.CREAR_FACTURA(@COMISIONES_ID AS BOBBY_TABLES.LISTA_INT READONLY,
	@FECHA_APP VARCHAR(30),@TARJETA VARCHAR(40),@TOTAL FLOAT) AS
BEGIN
	DECLARE @FECHA DATETIME, @TARJETA_HASH VARBINARY(20), @FACTURA NUMERIC(18,0)
	SET @FECHA = CONVERT(DATETIME, @FECHA_APP)
...
BEGIN
	...
	SELECT TOP 5 P.CLI_ID AS ID_Cliente, P.NOMBRE AS Nombre, P.APELLIDO AS Apellido, COUNT(C.CUE_ID) AS Comisiones, SUM(F.IMPORTE) AS Deuda_Total FROM BOBBY_TABLES.CLIENTES P, BOBBY_TABLES.CUENTAS C, BOBBY_TABLES.COMISIONES F, BOBBY_TABLES.ESTADOS_CUENTA E
